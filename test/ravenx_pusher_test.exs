defmodule RavenxPusherTest do
  ...
  setup do
    ...
    Application.put_env(:ravenx, :strategies, pusher: Ravenx.Strategy.Pusher)

    {:ok, bypass: bypass, payload: payload, opts: opts}
  end

  for field <- @required_options do
    test "`call/2` returns `{:error, {:missing_config, #{field}}` when #{field} is not present.",
         %{payload: payload, opts: opts} do
  ...
  for field <- @push_required do
    test "`call/2` returns `{:error, {:missing_field, #{field}}` when #{field} is not present.",
         %{payload: payload, opts: opts} do
  ...
  test "`Ravenx.dispatch/3` returs `{:error, {:pusher_error, %Push{}}}` when something goes wrong with pusher.",
       %{bypass: bypass, payload: payload} do
    Bypass.expect(bypass, fn conn -> Conn.resp(conn, 403, "") end)

    response = Ravenx.dispatch(:pusher, payload)

    assert {:error, {:pusher_error, %Push{} = _push}} = response
  end
  ...
  test "`Ravenx.dispatch/3` returs `{:ok, %Push{}}` when everything goes as planned.", %{
    bypass: bypass,
    payload: payload
  } do
  ...
  test "`Ravenx.dispatch/3` returs `{:ok, %Push{}}` overrides the options when passed.", %{
    bypass: bypass,
    payload: payload,
    opts: options
  } do
