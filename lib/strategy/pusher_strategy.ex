defmodule Ravenx.Strategy.Pusher do
  @moduledoc """
  ...
  import Pusher, only: [trigger: 4, configure!: 5]
  ...
  for field <- @required_options do
    defp send_push(_push, %{unquote(field) => f}) when f in [nil, ""] do
      {:error, {:missing_config, unquote(field)}}
    end
  end

  for field <- @push_required do
    defp send_push(%Push{unquote(field) => f}, _opts) when f in [nil, ""] do
